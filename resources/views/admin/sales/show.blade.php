@extends('admin.layouts.app', [
    'pageName' => __('trans.sale_details'),
])
           ...
           <div class="card-header bg-white border-bottom py-3 px-4">
            ...
            <div class="card-body">
                ...
                <div class="mb-4">
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            <strong>@lang('trans.invoice_number'):</strong>
                            <div class="text-muted">{{ $sale->invoice_number }}</div>
                        </div>
                        <div class="col-md-6">
                            <strong>@lang('trans.total'):</strong>
                            <div class="fw-bold text-success">{{ $sale->total }}</div>
                        </div>
                        <div class="col-md-6">
                            <strong>@lang('trans.sale_date'):</strong>
                            <div class="text-muted">{{ $sale->sale_date }}</div>
                        </div>
                         <div class="col-md-6">
                            <strong>@lang('trans.discount'):</strong>
                            <div class="fw-bold text-danger">{{ $sale->discount_value }}</div>
                        </div>
                        <div class="col-md-6">
                            <strong>@lang('trans.user'):</strong>
                            <div class="text-muted">{{ $sale->user->username }}</div>
                        </div>
                         <div class="col-md-6">
                            <strong>@lang('trans.net'):</strong>
                            <div class="fw-bold text-success">{{ $sale->net_amount }}</div>
                        </div>
                        <div class="col-md-6">
                            <strong>@lang('trans.client'):</strong>
                            <div class="text-muted">{{ $sale->client->name }}</div>
                        </div>
                        <div class="col-md-6">
                            <strong>@lang('trans.paid'):</strong>
                            <div class="fw-bold text-primary">{{ $sale->paid_amount }}</div>
                        </div>
                        <div class="col-md-6">
                            <strong>@lang('trans.safe'):</strong>
                            <div class="text-muted">{{ $sale->safe->name }}</div>
                        </div>
                        <div class="col-md-6">
                            <strong>@lang('trans.remaining'):</strong>
                            @if($sale->remaining_amount > 0)
                                <div class="fw-bold text-danger">{{ $sale->remaining_amount }}</div>
                            @else
